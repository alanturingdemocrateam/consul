...
describe "Machine learning" do
  ...
  scenario "Show output files info on settins page" do
    ...
    allow_any_instance_of(MachineLearning).to receive(:run) do
      ...
      File.open(comments_summary_file, "w") { |file| file.write([].to_json) }
    end
    ...
    expect(page).to have_content "Last execution #{info.generated_at.strftime("%Y/%m/%d - %H:%M")}"
    ...
    expect(page).to have_link "ml_comments_summaries.json",
                              href: MachineLearning.data_path("ml_comments_summaries.json")
  end
end
